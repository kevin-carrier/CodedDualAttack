...
channel_t* copy_channel(channel_t* c){
    channel_t *channel = (channel_t *)malloc(sizeof(channel_t));
    channel->std_dev = c->std_dev;
    channel->q = c->q;
    channel->distrib = (double *)malloc(sizeof(double) * channel->q);
    for(size_t i = 0; i < channel->q; ++i){
        channel->distrib[i] = c->distrib[i];
    }
    channel->rand_set = (unsigned long *)malloc(sizeof(unsigned long)*RAND_PRECISION);
    for(size_t i = 0; i < RAND_PRECISION; ++i){
        channel->rand_set[i] = c->rand_set[i];
    }
    return channel;
}
